...
VOID MY_PLAY_ENEMY_OPERATION_DETAIL(BOSS *boss, CHARADATA *c)
{
	...
	static int x = 0, y = 0;
		...
		int sa_y = Myplayer.Y  - boss->bossimage.Y[0];
		int sa_x = Myplayer.X - boss->bossimage.X[0];
		...
		if (sa_y_ab	!=0 &&sa_y_ab < sa_x_ab)					//横の移動幅が広い時
		{
			if (sa_y > 0)						//自然数のとき
			{
		...
		else if (sa_y_ab == 0 && sa_y_ab < sa_x_ab)
		{
			if (sa_x > 0)						//自然数のとき
			{
		...
		if (sa_x_ab !=0 &&sa_y_ab > sa_x_ab)								//縦の移動幅が広い時
		{
			if (sa_x > 0)						//自然数のとき
			{
				boss_move_dist = (int)BOSS_MOVE_RIGHT;	//敵の移動方向、右
		...
		else if (sa_x_ab == 0 && sa_y_ab > sa_x_ab)
		{
			if (sa_y > 0)						//自然数のとき
			{
				boss_move_dist = (int)BOSS_MOVE_DOWN;		//敵の移動方向、下
		...
		if (boss->bossimage.Y[0] == Myplayer.Y&&boss->bossimage.X[0] == Myplayer.X)
		{
			boss_move_dist = (int)BOSS_MOVE_NONE;	//移動方向なし
		}
	//else
	//{
